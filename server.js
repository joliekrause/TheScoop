...
function createComment(url, request) {
  ...
  const response = {};
  // Jolie
  // return 400 if no body, or username doesn't exist, or articleID doesn't exist
  // return 201 and comment if successful
  if (requestComment && requestComment.comment &&
...
function saveDatabase() {
  ...
  yaml.sync('thescoopdatabase.yml', database);
}
...
function loadDatabase() {
  ...
  fs   = require('fs');
  try {
    database = yaml.safeLoad(fs.readFileSync('thescoopdatabase.yml', 'utf8'));
    //console.log(doc);
  } catch (e) {
    console.log(e);
  }*/
  ...
  try {
    database = read.sync('thescoopdatabase.yml');
  } catch (e){
    // it would be nice to create the file here.
    fs   = require('fs');
    fs.appendFile('thescoopdatabase.yml','',function (err) {
  if (err) throw err;});
    console.log('here');
  }
} // loadDatabase
